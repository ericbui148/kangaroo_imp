...
bool Kangaroo::ParseConfigInline(std::string &startHex, std::string &endHex, std::string &pubkeyHex, std::string &address) {

  if(clientMode)
    return true;

  rangeStart.SetBase16((char *)startHex.c_str());
  rangeEnd.SetBase16((char *)endHex.c_str());

  Point p;
  bool isCompressed;
  if(!secp->ParsePublicKeyHex(pubkeyHex, p, isCompressed)) {
    ::printf("Error: invalid pubkey: %s\n", pubkeyHex.c_str());
    return false;
  }
  keysToSearch.push_back(p);
