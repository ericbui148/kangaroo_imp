...
void printUsage() {
  ...
  printf("         [-pubkey pubkeyHex] [-address btcAddress] [-start rangeStart] [-end rangeEnd]\n");
  printf("         [inFile]\n");
  ...
  printf(" -pubkey pubkeyHex: Public key (compressed hex) to search\n");
  printf(" -address btcAddress: Bitcoin address (display only, requires -pubkey)\n");
  printf(" -start rangeStart: Range start in hex (e.g. 4000000000000000000000000000000000)\n");
  printf(" -end rangeEnd: Range end in hex (e.g. 7fffffffffffffffffffffffffffffffff)\n");
  printf(" inFile: Input configuration file (not needed when -pubkey/-start/-end are given)\n");
  exit(0);
...
static string pubkeyArg = "";
static string addressArg = "";
static string rangeStartArg = "";
static string rangeEndArg = "";
...
int main(int argc, char* argv[]) {
  ...
  while (a < argc) {
    ...
    if(strcmp(argv[a], "-t") == 0) {
      ...
      CHECKARG("-pubkey",1);
      pubkeyArg = string(argv[a]);
      a++;
    } else if(strcmp(argv[a],"-address") == 0) {
      CHECKARG("-address",1);
      addressArg = string(argv[a]);
      a++;
    } else if(strcmp(argv[a],"-start") == 0) {
      CHECKARG("-start",1);
      rangeStartArg = string(argv[a]);
      a++;
    } else if(strcmp(argv[a],"-end") == 0) {
      CHECKARG("-end",1);
      rangeEndArg = string(argv[a]);
      a++;
    } else if(a == argc - 1) {
  ...
  if(checkFlag) {
    ...
    if(checkWorkFile.length() > 0) {
      ...
      if( !v->ParseConfigFile(configFile) )
        exit(-1);
    } else if(pubkeyArg.length()>0) {
      if(rangeStartArg.empty() || rangeEndArg.empty()) {
        ::printf("Error: -pubkey requires -start and -end\n");
        exit(-1);
      }
      if( !v->ParseConfigInline(rangeStartArg, rangeEndArg, pubkeyArg, addressArg) )
        exit(-1);
    } else {
      if(serverIP.length()==0) {
        ::printf("No input file to process\n");
        ::printf("Use: kangaroo -pubkey <hex> -start <hex> -end <hex>\n");
        ::printf("  or provide an input file\n");
        exit(-1);
      }
    }
    if(serverMode)
